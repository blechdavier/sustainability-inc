export const TUTORIAL = [
    {
        title: "Tutorial",
        description: "In Sustainability Inc., you're the CEO of a small company that sells different things. You get points for making money, but lose them for emitting CO2.",
    },
    {
        title: "Buy Products",
        description: "Click a product to buy it. The products will be sold to your customers, and you'll make money.",
    },
    {
        title: "Currently Selling",
        description: "When you click a product, it goes here, where some customer buys it after a fixed amount of time."
    },
    {
        title: "Money and Emissions",
        description: "In this section, you can keep track of how much money you've made and how much CO2 you've emitted."
    },
    {
        title: "Score",
        description: "This is your score. Be mindful of your emissions, and have fun!"
    },
]